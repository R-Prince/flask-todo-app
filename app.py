...
def login():
    if request.method == "POST":
        ...
        if existing_user:
            # check hash password matches user input
...
def add_task():
    ...
    categories = mongo.db.catergories.find().sort("category_name", 1)
...
def get_categories():
    categories = list(mongo.db.catergories.find().sort('categories_name', 1))
...
def add_category():
    if request.method == "POST":
        ...
        mongo.db.catergories.insert_one(category)
...
def edit_category(category_id):
    if request.method == "POST":
        ...
        mongo.db.catergories.update({"_id": ObjectId(category_id)}, submit)
...
def delete_category(category_id):
    mongo.db.catergories.remove({'_id': ObjectId(category_id)})
